...
##### overalp rs ##### 
#$tabix $kgsnp ${chr}:${minpos}-${maxpos} |awk -v OFS="\t" '!a[$NF]++ && $NF!~/;/ && $NF~/^rs/{print $NF,$2}'|awk -F "\t" -v OFS="\t" -v gwasfile=$gwasfile -v qtlfile=$qtlfile  'BEGIN{while((getline < gwasfile )>0) a[$5]=1; while((getline < qtlfile) >0) b[$4]=0;}{if(($1 in a) && ($1 in b))  print $1,$2 }' >common.snp.full
#$tabix $kgsnp ${chr}:${minpos2}-${maxpos2} |awk -v OFS="\t" '!a[$NF]++ && $NF!~/;/ && $NF~/^rs/{print $NF,$2}'|awk -F "\t" -v OFS="\t" -v gwasfile=$gwasfile -v qtlfile=$qtlfile  'BEGIN{while((getline < gwasfile )>0) a[$5]=1; while((getline < qtlfile) >0) b[$4]=0;}{if(($1 in a) && ($1 in b))  print $1,$2 }' >common.snp.full2
#cat <(awk -F "\t" -v OFS="\t" 'NR>1{print $5,$2}' $gwasfile) <(awk -F "\t" -v OFS="\t" 'NR>1{print $4,$6}' $qtlfile) |awk -F "\t" -v OFS="\t" 'a[$0]++ && $1~/rs/' |sort -k2n >common.snp.full

gzcat $ldfile |awk -v OFS="\t" '$3~/rs/{print $3,$1}' |awk -F "\t" -v OFS="\t" -v gwasfile=$gwasfile -v qtlfile=$qtlfile  'BEGIN{while((getline < gwasfile )>0) a[$5]=1; while((getline < qtlfile) >0) b[$4]=0;}{if(($1 in a) && ($1 in b))  print $1,$2 }' |sort -k2n  >$tmpfold/common.snp.full
...
if [ "$commonlead" -ne 1 ] ; then
	...
	leadsnp=`awk -F "\t" -v OFS="\t" -v pos=$position '$2>pos{print $1; exit 0;}' common.snp.full ` 
	echo "$leadsnp as leadsp" >>$tmpfold/run.log
	leadsnp_included="N"
	
fi

grep -C 50 $leadsnp $tmpfold/common.snp.full|cut -f 1 >$tmpfold/common.snp.50
...
position=`grep $leadsnp $tmpfold/common.snp.full|awk '{print $2}' `		
cat $tmpfold/common.snp.full|awk -v pos=$position -v dist=$dist '$2>=(pos-dist) && $2<=(pos+dist){print $1}' >$tmpfold/common.snp 
...
Rscript $LD_subset ${ldfile} $tmpfold/common.snp $tmpfold/common.snp.50 $tmpfold/emerald.LD.txt $tmpfold/emerald.50.LD.txt
