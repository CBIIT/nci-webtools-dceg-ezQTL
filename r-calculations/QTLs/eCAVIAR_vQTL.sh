...
##### overalp rs ##### 
$tabix $kgsnp ${chr}:${minpos}-${maxpos} |awk -v OFS="\t" '!a[$NF]++ && $NF!~/;/ && $NF~/^rs/{print $NF,$2}'|awk -F "\t" -v OFS="\t" -v gwasfile=$gwasfile -v qtlfile=$qtlfile  'BEGIN{while((getline < gwasfile )>0) a[$5]=1; while((getline < qtlfile) >0) b[$4]=0;}{if(($1 in a) && ($1 in b))  print $1,$2 }' >tmp/common.snp.full
$tabix $kgsnp ${chr}:${minpos2}-${maxpos2} |awk -v OFS="\t" '!a[$NF]++ && $NF!~/;/ && $NF~/^rs/{print $NF,$2}'|awk -F "\t" -v OFS="\t" -v gwasfile=$gwasfile -v qtlfile=$qtlfile  'BEGIN{while((getline < gwasfile )>0) a[$5]=1; while((getline < qtlfile) >0) b[$4]=0;}{if(($1 in a) && ($1 in b))  print $1,$2 }' >tmp/common.snp.full2

### if no gwas leadsnp in eQTL file, use the neary-by SNPs. and set up the tag for this
leadsnp_included="Y"
commonlead=$(grep $leadsnp tmp/common.snp.full |wc -l)

if [ "$commonlead" -ne 1 ] ; then
	echo "warning: the orignal lead snp $leadsnp are not found, use the nearby one:" >>tmp/run.log
	leadsnp=`awk -F "\t" -v OFS="\t" -v pos=$position '$2>pos{print $1; exit 0;}' tmp/common.snp.full ` 
	echo "$leadsnp as leadsp" >>tmp/run.log
	leadsnp_included="N"
	
fi

grep -C 50 $leadsnp tmp/common.snp.full2|cut -f 1 >tmp/common.snp.50
snp50num=`wc -l tmp/common.snp.50|awk '{print $1}' `

cut -f 1 tmp/common.snp.full |cat - tmp/common.snp.50 |awk -F "\t" -v OFS="\t" 'NR==FNR{a[$1]=1;next;}$1 in a {print $1}' - tmp/common.snp.full2 >tmp/common.snp 
...
$bcftools view -S $poppanel -O v $kgvcfpath ${chr}":"${minpos}"-"${maxpos}|awk -F "\t" -v OFS="\t" 'NR==FNR{a[$1]=1;next;}/^#/ || $3 in a {print $0}' tmp/common.snp -|$bcftools sort -O z -o tmp/input.vcf.gz 
$bcftools index -t tmp/input.vcf.gz
$emeraLD --matrix -i tmp/input.vcf.gz --out tmp/emerald

$bcftools view tmp/input.vcf.gz |awk -F "\t" -v OFS="\t" 'NR==FNR{a[$1]=1;next;}/^#/ || $3 in a {print $0}' tmp/common.snp.50 - |$bcftools sort -O z -o tmp/input50.vcf.gz
$bcftools index -t tmp/input50.vcf.gz
$emeraLD --matrix -i tmp/input50.vcf.gz --out tmp/emerald.50

cat $gwasfile |cut -f 5,7|awk -F "\t" -v OFS="\t" 'NR==FNR{a[$1]=$0;next;}{print a[$1]}' - tmp/common.snp  >tmp/GWAS.z
cat $gwasfile |cut -f 5,7|awk -F "\t" -v OFS="\t" 'NR==FNR{a[$1]=$0;next;}{print a[$1]}' - tmp/common.snp.50  >tmp/GWAS.z.50
...
if [ "$eqtlpeaksnp" -ne 1 ] || [ "$gwaspeaksnp" -ne 1 ] ; then
	echo "$prefix: no eQTL found for GWAS lead SNP $leadsnp and gene $gene, or no this leadsnp found in gwas file, abort......" >>tmp/run.log
	#exit 0
	continue
fi
...
if [ "$snp50num" -ne 101 ] ; then
	echo "Warning not enough 50 snps for testing, abort......" >>tmp/run.log
	#exit 0
	#continue
fi
