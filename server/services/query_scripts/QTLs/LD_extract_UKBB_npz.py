...
def Parse():
	...
	parser.add_argument('-q', '--query_postion', required=True, type=str, help="query_postion")
	#parser.add_argument('-l', '--LD_File_Dir', required=True, type=str, help="the Dir Path where LD files were located")
	parser.add_argument('-o', '--output_Path', required=True, type=str, help="output_Path")
	parser.add_argument('-r', '--Path_LD_File_List', required=True, type=str, help="output_dir")
...
def load_NPZ(query_Start, query_End, return_Targated_Str):
	...
	gz_npz_np_T_temp = gz_npz_np_temp + gz_npz_np_temp.T - np.diag(np.diag(gz_npz_np_temp))
	...
	gz_Final_pd_Part2 = pd.concat([gz_Final_pd_Part1,gz_npz_pd_T_temp],axis=1)
	
	####### gzip the Ouput_Path
	Output_Path = "%s/Output_%s.txt" % (output_dir,query_postion.replace(":","-"))
...
def Process():
	...
	if return_Targated_Str == "0":
		Error_msg = "Your Input Region %s is invalid, please provide another position in right format, such as chr9:100060001-103000001" % (query_postion)
		print(Error_msg)
	else:
...
def Test():
	...
	LD_gz_Path = tf.keras.utils.get_file('chr10_10000001_13000001.gz', Test_URL_gz)
