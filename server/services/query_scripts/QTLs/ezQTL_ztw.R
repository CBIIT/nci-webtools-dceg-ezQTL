...
coloc_QC <- function(gwasfile = NULL, gwasfile_pub = FALSE, qtlfile = NULL, qtlfile_pub = FALSE, ldfile = NULL, ldfile_pub = FALSE, leadsnp = NULL, leadpos = NULL, distance = 100000, zscore_gene = NULL, output_plot_prefix = NULL, output_prefix = "./ezQTL_input", logfile = "ezQTL.log") {
  ...
  if (!is.null(qtlfile)) {
    if (file.info(qtlfile)$size < 50) {
      ...
      if (length(qtl_colname_diff) != 0) {
        ...
        qtltmp <- qtl %>%
          select(variant_id, rsnum, chr, pos, ref, alt) %>%
          unique()
  ...
  if (!is.null(leadsnp) && !is.na(leadsnp)) {
    ...
    if (!is.null(qtlfile)) {
      qtl <- qtl %>% filter(chr == leadchr, pos >= leadpos1, pos <= leadpos2)
    }
  ...
  if (!is.null(qtlfile)) {
    ...
    pall0 <- ggplot() +
      geom_text(aes(x = 1, y = 1, label = labeltext), family = "Roboto Condensed", size = 5) +
      theme_nothing()
    # pall0 <- ggplot()
  ...
  if (!is.null(gwasfile) & !is.null(qtlfile) & !is.null(ldfile)) {
    ...
    qtl2 <- qtl %>%
      group_by(rsnum, pos) %>%
      arrange(pval_nominal) %>%
      summarise(pvalue = min(pval_nominal), n = n()) %>%
      ungroup()
    ndifgene <- qtl %>%
      count(gene_id) %>%
      count(n) %>%
    ...
    qtl_rm <- qtl %>%
      mutate(diff = abs(pos - leadpos), direction = if_else(pos > leadpos, "+", "-")) %>%
      filter(diff != 0) %>%
  ...
  p <- ggplot(df) +
    ...
    scale_fill_tl_distiller(
      ...
      guide = guide_colourbar(
        direction = "vertical", barheight = unit(6, "cm"),
        order = 1,
        title.position = "top"
      )
    ) +
    ...
    theme(
      panel.background = element_rect(fill = "grey25"), axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5),
...
locus_quantification_dis <- function(qdata, qtldata, genesets = NULL, output_plot = NULL, plot_width = NULL, plot_height = NULL) {
  ...
  genesets2 <- id2symbol %>%
    filter(gene_symbol %in% genesets) %>%
    pull(gene_id)
  qdata2 <- qdata %>%
  ...
  genelevel <- qdata2 %>%
    group_by(gene_symbol) %>%
    summarise(mexp = median(expression, na.rm = TRUE)) %>%
    arrange(mexp) %>%
    pull(gene_symbol)
...
IntRegionalPlot <- function(chr = NULL, left = NULL, right = NULL, association_file = NULL, trait = NULL, LDfile, genome_build = "GRCh37", gtf_tabix_file, Distance = 50000, Distance_max = 2000000, output_file = NULL,
  ...
  ld.matrix <- read_delim(LDfile, delim = "\t", col_names = F, col_types = cols("X1" = "c")) %>% rename(chr = X1, pos = X2, rsnum = X3, ref = X4, alt = X5)
  ld.info <- ld.matrix %>%
    dplyr::select(chr, pos, rsnum, ref, alt) %>%
    mutate(Seq = seq_along(chr))
  ld.matrix <- ld.matrix %>%
    dplyr::select(-c(chr, pos, rsnum, ref, alt)) %>%
    as.matrix()
  ...
  if (is.null(association_file)) {
    ...
    if (!is.null(leadsnp)) {
      association <- ld.info %>%
        dplyr::mutate(p = if_else((rsnum == leadsnp), 1e-10, 0.05)) %>%
        dplyr::select(Marker = rsnum, Locus = chr, Site = pos, p)
    }
    if (!is.null(leadsnp_pos)) {
      association <- ld.info %>%
        dplyr::mutate(p = if_else((pos == leadsnp_pos), 1e-10, 0.05)) %>%
        dplyr::select(Marker = rsnum, Locus = chr, Site = pos, p)
    }
  } else {
    ...
    if ("pvalue" %in% colnames(association)) {
      association <- association %>% select(Marker = rsnum, Locus = chr, Site = pos, p = pvalue)
    }
    if ("pval_nominal" %in% colnames(association)) {
      ...
      association <- association %>%
        filter(gene_symbol == trait) %>%
        select(Marker = rsnum, Locus = chr, Site = pos, p = pval_nominal)
    }
  }


  if (any(is.null(chr), is.null(left), is.null(right))) {
    tmplocus <- association %>%
      arrange(p) %>%
      slice(1)
    chr <- tmplocus$Locus
  ...
  if (dim(transcript_association)[1] < 2) {
    ...
    if (all(length(threshold) > 0, threshold <= max(pvalue_range))) {
      threshold_line <- list(geom_segment(aes(
    ...
    if (any(isTRUE(leadsnpLD), isTRUE(triangleLD))) {
      ...
      ld <- data.frame(
        Var1 = c(as.character(ld$Var1), as.character(ld$Var2)),
      ...
      poly_data$value <- ld$value[match(
        paste0(poly_data$Var1, "/", poly_data$Var2),
  ...
  if (is.null(association_file)) {
    ...
    if (dim(ld_leadsnp)[1] == 0) {
      plot <- plot + geom_point(data = transcript_association, aes(Site, -log10(p) * fold), shape = 21, colour = "black", fill = "white", size = 2)
    }
  } else {
    plot <- ggplot() +
      threshold_line +
  ...
  if (is.null(output_file)) {
    ...
    if (str_detect(output_file, "png$")) {
      ggsave(filename = output_file, plot = plot, width = 16, height = 16, dpi = "retina")
    }
...
gwas2qtl <- function(gwas) {
  nqtl <- gwas %>%
    mutate(gene_id = "GWAS", gene_symbol = "GWAS", variant_id = paste0(chr, pos, ref, alt), tss_distance = NA_integer_) %>%
