...
export function LocusTable() {
  ...
  const columns = [
    ...
    },
    {
      ...
      disableSortBy: true,
      Cell: row => (
        <a href={"https://ldlink.nci.nih.gov/?tab=ldpop&var1=" + row.value + "&var2=" + locus_alignment['top']['rsnum'] + "&pop=" + inputs['select_pop'][0].split('+').join('%2B') + "&r2_d=r2"} target="_blank" rel="noreferrer">Go to</a>
      )
    },
    {
      Header: 'NHGRI GWAS',
      ...
      disableSortBy: true,
      Cell: row => (
        <a href={"https://www.ebi.ac.uk/gwas/search?query=" + row.value} target="_blank" rel="noreferrer">Go to</a>
      )
    },
    {
      Header: 'gnmomAD',
      ...
      disableSortBy: true,
      Cell: row => (
        <a href={"http://gnomad.broadinstitute.org/variant/" + row.value.chr + "-" + row.value.pos + "-" + row.value.ref + "-" + row.value.alt} target="_blank" rel="noreferrer">Go to</a>
      )
    },
  ];
