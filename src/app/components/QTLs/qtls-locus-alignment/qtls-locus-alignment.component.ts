  ...
  ngOnInit() {
    ...
    this.data.currentMainData.subscribe(mainData => {
      if (mainData) {
        ...
        console.log("INIT NEW SELECTED POP", this.selectedPop);
        this.selectedGene = this.newSelectedGene; // recalculated new gene selection
        console.log("INIT NEW SELECTED GENE", this.selectedGene);
      }
    ...
    if (event.points) {
      ...
      if (event.points[0].hasOwnProperty("marker.color")) {
        ...
        if (this.collapseInput) { // input panel collapsed
          if (this.GWASData[0] && this.locusAlignmentScatterData[0]) { // GWAS scatter shown
            $('.popover').show().css({
              position: "absolute",
              top: top + 585, 
              left: left + 190
